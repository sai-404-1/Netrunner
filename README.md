...
NetRunner — инструмент централизованного управления парком Linux-компьютеров: массовые
удалённые команды, многошаговые сценарии, плановое выполнение и автообслуживание машин
через встроенного endpoint-агента. Подходит и для администратора с десятком серверов,
и для сетевого/системного инженера, обслуживающего флот рабочих станций.

Оператор ведёт реестр хостов, группирует их и выполняет против одного хоста, группы или
всего парка: разовые команды, инвентаризацию и произвольные SSH-действия — по одному или
сразу массово, с живым статусом, отменой, расписанием и экспортом отчётов.
...
- **Сценарии** — многошаговые последовательности модулей, выполняемые **пер-хост**:
  каждый компьютер идёт по своей очереди шагов независимо, со своим `on_failure`
  (продолжить/остановить), контролем темпа (пакетно или с ограничением параллелизма)
  и историей запусков.
- **Запуск задач** — асинхронное исполнение по каждому хосту с живым статусом,
  опросом результата и **отменой** выполняющейся задачи.
- **Планировщик** — плановые задачи, каждая из которых запускает сценарий:
  - разовый запуск по конкретному времени;
  - повторяющееся **расписание** (cron): выбранные дни недели + окно «С…До» + интервал;
  - условие «**когда цель будет в сети**» — задача ждёт включения машины и выполняется
    сама, как только та появится (важно для автообслуживания офлайн-парка).
- **Endpoint-агент** — фоновый процесс на каждой машине, держит WS-соединение с сервером
  (живой статус онлайн/офлайн, heartbeat) и позволяет переустанавливать/перепривязывать
  себя удалённо без ручного обхода.
- **Единая история** — общая лента событий: запуски сценариев и задач, сообщения агента,
  автозапуски планировщика.
...
- `agent/` — исходники **endpoint-агента**: фоновый процесс, который ставится на управляемые
  машины и держит WS-соединение с сервером (статус онлайн, heartbeat, удалённые команды).
- `database/` — лёгкий ORM-подобный слой поверх `sqlite3` (схема, модели, репозитории).
- `services/` — бизнес-логика: контекст приложения, task-runner, раннер сценариев,
  планировщик (расписание/ожидание сети), auth, реестр модулей, история, отчёты.
...
- `GET/POST /api/scenarios` — сценарии (список, запуск `POST /api/scenarios/run`).
- `GET/POST /api/schedule*` — запланированные задачи планировщика (создание, список,
  ручной тик `/api/scheduler/tick`).
...
- `WS /ws` — живые обновления статуса задач; endpoint-агенты держат здесь свои соединения.
