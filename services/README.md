...
- `class ExecutionSettings(db)` — глобальные настройки исполнения на сервере:
  ...
  - **адрес агента** `agent_ws_url` (`ws://`/`wss://`) и **корень сервера** `agent_ca_cert`
    (PEM для `wss://`; `ca_cert_info()` принимает ровно один сертификат с CA:TRUE, закрытый
    ключ отвергает, отдаёт subject/срок/SHA-256 для формы).
...
- `class AgentService` — жизненный цикл endpoint-агента на управляемой машине:
  ...
  ключа/токена, `token_for_config` — открытый токен для конфига агента при переустановке
  (не расшифровался — выпускается новый), конфиг WS-адреса (`NETRUNNER_AGENT_WS_URL` в Docker).
